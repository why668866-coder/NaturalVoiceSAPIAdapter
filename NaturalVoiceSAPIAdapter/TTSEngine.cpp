...
#include "RegKey.h"
#include "wrappers.h"
#include <charconv> // 确保包含 charconv
...
bool CTTSEngine::InitCloudVoiceSynthesizer(ISpDataKey* pConfigKey)
{
    ...
    if (!proxy.empty())
    {
        ...
        // 【核心修复】使用 std::stoul 替代 std::from_chars，彻底避免 .size() 和 from_chars 的编译错误
...
void CTTSEngine::SetupRestAPIEvents(ULONGLONG interests)
{
    ...
    if ((interests & SVEWordBoundary)
        || (m_isEdgeVoice && (interests & SVEBookmark))) 
...
static std::wstring_view GetXMLTagName(const std::wstring& tag)
{
    ...
    auto end = std::find_if(begin, tag.end() - 1, iswspace); 
...
static bool IsXMLClosingTag(std::wstring_view tag)  
{
...
static bool IsXMLSelfClosingTag(std::wstring_view tag)  
{
    if (tag.size() < 4) return false;  
            ...
            )
        {
            ...
            if (volume != 100 || rate != 0 || pitch != 0) 
            {
                ...
                if (volume != 100)
                {
                    ...
                    m_ssml.append(std::to_wstring(volume - 100)); 
                    m_ssml.append(L"%'");
                }
                if (rate != 0)
                {
                    m_ssml.append(L" rate='");
                    m_ssml.append(std::to_wstring(rate >= 0 ? rate * 20 : rate * 20 / 3)); 
                    m_ssml.append(L"%'");
                }
                if (pitch != 0)
                {
                    m_ssml.append(L" pitch='");
                    m_ssml.append(std::to_wstring(pitch * 5)); 
                    m_ssml.append(L"%'");
                }
                m_ssml.push_back('>');
                isInProsodyTag = true;
            ...
            case SPVA_ParseUnknownTag: 
            {
                ...
                else if (IsXMLClosingTag(tag))
                {
                    ...
                    if (tagToClose != customTags.rend())
                    {
                        ...
                        m_ssml.append(pTextFrag->pTextStart, pTextFrag->ulTextLen);
                        customTags.erase(tagToClose.base() - 1, customTags.end());  
                    }
                    else
                    {
                ...
                else if (pTextFrag->ulTextLen >= 3) 
                {
                    m_ssml.append(pTextFrag->pTextStart, pTextFrag->ulTextLen);
                    customTags.emplace_back(pTextFrag->pTextStart, pTextFrag->ulTextLen);  
                }
                else
                {
...
void CTTSEngine::MapTextOffset(ULONG& ulSSMLOffset, ULONG& ulTextLen)
{
    ...
    while (index > 0
        && m_offsetMappings[index - 1].ulSSMLTextOffset == m_offsetMappings[index].ulSSMLTextOffset  
        && m_offsetMappings[index - 1].ulSAPITextOffset >= endSAPIOffset  
        )
        index--;
